...
import CredibleEditor, { InteractiveCredibleEditor, Context } from '../src/ckeditor';
...
describe( 'Context', () => {
	...
	it( 'is a context', () => {
		expect( Context ).to.be.instanceof( CKContext )
	})
})

describe( 'CredibleEditor build', () => {
	...
	describe( 'build', () => {
		it( 'contains plugins', () => {
			expect( CredibleEditor.builtinPlugins ).to.not.be.empty;
		} );

		it( 'contains config', () => {
			expect( CredibleEditor.defaultConfig.toolbar ).to.not.be.empty;
		} );
	} );

	describe( 'create()', () => {
		beforeEach( () => {
			return CredibleEditor.create( editorElement )
				.then( newEditor => {
					editor = newEditor;
				} );
		} );
		...
		it( 'creates an instance which inherits from the CredibleEditor', () => {
	...
	describeMemoryUsage( () => {
		testMemoryUsage(
			...
			() => CredibleEditor.create( document.querySelector( '#mem-editor' ) ) );
	} );
} );

describe( 'InteractiveCredibleEditor build', () => {
	let editor, editorElement;
	...
	describe( 'build', () => {
		it( 'contains plugins', () => {
			expect( InteractiveCredibleEditor.builtinPlugins ).to.not.be.empty;
		} );

		it( 'contains config', () => {
			expect( InteractiveCredibleEditor.defaultConfig.toolbar ).to.not.be.empty;
		} );
	} );

	describe( 'create()', () => {
		beforeEach( () => {
			return InteractiveCredibleEditor.create( editorElement )
				.then( newEditor => {
					editor = newEditor;
				} );
		} );
		...
		it( 'creates an instance which inherits from the InteractiveCredibleEditor', () => {
			expect( editor ).to.be.instanceof( ClassicEditor );
			expect( editor ).to.be.instanceof( BaseClassicEditor );
		} );
	...
	describe( 'config', () => {
		...
		// https://github.com/ckeditor/ckeditor5/issues/572
		it( 'allows configuring toolbar items through config.toolbar', () => {
			return InteractiveCredibleEditor
				.create( editorElement, {
					toolbar: [ 'bold' ]
				} )
				.then( newEditor => {
	...
	describeMemoryUsage( () => {
		testMemoryUsage(
			'should not grow on multiple create/destroy',
			() => InteractiveCredibleEditor.create( document.querySelector( '#mem-editor' ) ) );
	} );
} );
